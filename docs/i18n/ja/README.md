...
NOFX は複数の AI モデルプロバイダーに対応しています。DeepSeek、OpenAI、Google などへ自分の API キーで接続でき、任意の OpenAI 互換エンドポイント(セルフホスト含む)も利用できます。
...
| **AI モデルアクセス** | 自分の API キーで複数プロバイダーに接続(OpenAI 互換エンドポイント対応) |
